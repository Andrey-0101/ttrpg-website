...
type CampaignHandoutsPageProps = {
  params: Promise<{ locale: string; id: string }>;
};

export async function generateMetadata({
  params,
}: CampaignHandoutsPageProps): Promise<Metadata> {
  ...
  return { title: translations("campaignHandouts") };
}

export default async function CampaignHandoutsPage({
  params,
}: CampaignHandoutsPageProps) {
  ...
  if (campaignError) {
    console.error("Failed to load the Campaign Handouts route.");
  }
  ...
  const { data: imageRows, error: imageError } = await supabase
    ...
    .select(
      "id, display_name, storage_object_name, visibility, created_at",
    )
    .eq("campaign_id", campaign.id)
    .order("created_at", { ascending: false });

  if (imageError) {
    console.error("Failed to load Campaign Handouts.");
  }
  ...
  const imagesWithUrls = await Promise.all(
    (imageRows ?? []).map(async (image) => {
      ...
      if (error) {
        console.error("Failed to create a Campaign Handout display URL.");
      }
  ...
  if (isGameMaster && !imageError) {
    ...
    if (recipientsResult.error || membersResult.error || profilesResult.error) {
      ...
      console.error("Failed to load Campaign Handout access controls.");
    } else {
