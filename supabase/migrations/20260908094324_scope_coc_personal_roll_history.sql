-- Retain one current roll plus five previous rolls per personal roller page.
-- Call of Cthulhu shares one retention scope across its two roller kinds.
...
begin
  ...
  retention_kinds := case
    when p_roller_kind = any(array[
      'coc_7e_percentile',
      'coc_7e_other_dice'
    ]) then array[
      'coc_7e_percentile',
    ...
    else array[p_roller_kind]
  end;

  delete from public.personal_roll_history as history
  where history.owner_id = current_owner_id
    and history.roller_kind = any(retention_kinds)
    and history.sequence_number < (
      ...
      where retained.owner_id = current_owner_id
        and retained.roller_kind = any(retention_kinds)
