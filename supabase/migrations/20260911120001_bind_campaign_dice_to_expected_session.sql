...
drop function public.record_campaign_dice_roll(uuid, uuid, text, jsonb, jsonb);
...
begin
  if target_actor_id is null
     or target_game_session_id is null
  ...
  if not found or session_row.ended_at is not null then
    return;
  end if;
...
revoke all on function public.record_campaign_dice_roll(uuid, uuid, uuid, text, jsonb, jsonb)
  from public, anon, authenticated, service_role;
grant execute on function public.record_campaign_dice_roll(uuid, uuid, uuid, text, jsonb, jsonb)
  to service_role;
